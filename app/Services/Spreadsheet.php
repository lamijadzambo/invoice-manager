        ...
        if (count($dbOrders) > 0) {
            ...
            foreach ($transformedOrders as $order) {
                $orders = [
                    ...
                    (string)$order->hg005 ?: (string)$order->england,
                    (string)$order->redMask ?: (string)$order->austria,
                    (string)$order->doorHandler ?: (string)$order->portugal,
                    isset($columnLimit) ? (string)$order->order_total_amount : (string)$order->medEinweg,
                    (string)$order->stoff,
