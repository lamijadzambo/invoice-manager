        ...
        if (count($dbOrders) > 0) {
            ...
            foreach ($transformedOrders as $order) {
                $orders = [
                    ...
                    (string)$order->ffp2 ?: (string)$order->netherlands,
                    (string)$order->ffp3 ?: (string)$order->spain,
                    (string)$order->childMask ?: (string)$order->england,
                    (string)$order->hg005 ?: (string)$order->austria,
                    (string)$order->redMask ?: (string)$order->portugal,
                    isset($columnLimit) ? (string)$order->order_total_amount : (string)$order->doorHandler,
                    (string)$order->medEinweg,
                    (string)$order->stoff,
